...
# augment function
##################################
def batch_augment(images, attention_map, mode='c', theta=0.5, padding_ratio=0.1):
    ...
    if mode == 'c':
        ...
        for batch_index in range(batches):
            ...
            c_mask = F.upsample_bilinear(atten_map, size=(imgH, imgW)) >= theta_c
            nonzero_indices = torch.nonzero(c_mask[0, 0, ...])
            ...
            c_images.append(
                F.upsample_bilinear(images[batch_index:batch_index + 1, :, height_min:height_max, width_min:width_max],
                                    size=(imgH, imgW)))
        c_images = torch.cat(c_images, dim=0)
        return c_images
        ...
        for batch_index in range(batches):
            ...
            d_masks.append(F.upsample_bilinear(atten_map, size=(imgH, imgW)) < theta_d)
        d_masks = torch.cat(d_masks, dim=0)
        d_images = images * d_masks.float()
        return d_images

    else:
        raise ValueError('Expected mode in [\'c\', \'d\'], but received unsupported augmentation method %s' % mode)
